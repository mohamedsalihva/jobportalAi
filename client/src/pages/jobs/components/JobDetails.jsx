      ...
      <div className="p-6 border-b border-slate-100 dark:border-white/10">
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex flex-wrap items-center gap-2 sm:gap-3 text-sm text-slate-600 dark:text-slate-300 font-medium">
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              <Tag
                ...
                text={job.salary || "Not disclosed"}
              />
              <Tag
                text={`Experience: ${job.experienceRequired || "0-2 yrs"}`}
              />
              {job.languages?.length > 0 && (
                <Tag
                  text={`Languages: ${job.languages.slice(0, 2).join(", ")}`}
                />
              )}
            </div>
            ...
            {isMyOwnJob && (
              <div className="mt-4 text-xs font-semibold text-amber-700 dark:text-amber-300 bg-amber-500/10 border border-amber-500/20 rounded-2xl px-4 py-3">
                ⚠️ You posted this job as a recruiter. You can’t apply to your
                own job.
              </div>
            )}
          </div>
          ...
          <div className="flex flex-col gap-2 w-full sm:w-auto shrink-0">
            ...
            <button
              onClick={onSave}
              className={`w-full sm:w-auto px-4 py-2.5 rounded-2xl border font-semibold text-sm transition flex items-center justify-center gap-2
              ${
                isSaved
            ...
            <button
              ...
              className={`w-full sm:w-auto px-4 py-2.5 rounded-2xl font-semibold text-sm transition
  ${
    isApplied
      ? "bg-emerald-600 text-white cursor-not-allowed opacity-90"
      : isMyOwnJob
        ? "bg-slate-300 dark:bg-white/10 text-slate-600 dark:text-slate-300 cursor-not-allowed"
        : "bg-amber-500 hover:bg-amber-400 text-black shadow-md shadow-amber-500/20"
  }`}
            >
              {isMyOwnJob ? "Your Job" : isApplied ? "Applied" : "Apply Now"}
            </button>
          </div>
        </div>
      </div>
